        ...
        for (int i = 0; i <sequence.length; i++) {
          ...
          if (scene.containsKey('players')) {
            ...
            players.forEach((playerId, playerData) {
              ...
              List<dynamic> positionTransformed = playerData['position_transformed'];
              double dy =(60- positionTransformed[0])/60  ; // Normalize to 0-1 range
              double dx = positionTransformed[1] / 90; // Normalize to 0-1 range
              
              // Convert team to color (example mapping)
              int color = playerData['team'] == 0 ? 4278255360 : 4294901760; // Green for team 0, Red for team 1
              
              // Create player object in desired format
              Map<String, dynamic> playerObject = {
                "color": color,
                "number":  int.parse(playerId) + 1, // Use playerId as number or adjust as needed
                "position": {
                  "dx": dx,
                  "dy": dy
                },
          ...
          if (scene.containsKey('goalkeeper')) {
            scene['goalkeeper'].forEach((keeperId, keeperData) {
              List<dynamic> positionTransformed = keeperData['position_transformed'];
              double dy =(60- positionTransformed[0])/60  ; // Normalize to 0-1 range
              double dx = positionTransformed[1] / 90; 
              Map<String, dynamic> keeperObject = {
                "color": 4278190080, // Black color for goalkeeper
                "number":int.parse(keeperId) + 1 , // Using 3 for goalkeeper as in your example
                "position": {
                  "dx": dx,
                  "dy": dy
                },
